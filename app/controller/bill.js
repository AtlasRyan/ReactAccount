...
class BillController extends Controller {
  async add() {
    const { ctx, app } = this;
    // 获取请求中携带的参数
    const { amount, label_id, date, remarks = '' } = ctx.request.body;

    // 判空处理，这里前端也可以做，但是后端也需要做一层判断。
    if (!amount || !label_id || !date) {
      ctx.body = {
        code: 400,
        msg: '参数错误',
        data: null,
  ...
  async list() {
    const { ctx, app } = this;
    // 获取，日期 date，分页数据，类型 label_id，这些都是我们在前端传给后端的数据
    const { date, page = 1, page_size = 5, label_id = 'all' } = ctx.query;
    try {
      let user_id;
      ...
      const _list = list.filter(item => {
        if (label_id !== 'all') {
          return moment(Number(item.date)).format('YYYY-MM') === date && label_id == item.label_id; //注意此处类型转换
        }
  ...
  async update() {
    ...
    const { id, amount, label_id, date, remarks = '' } = ctx.request.body;
    // 判空处理
    if (!amount || !label_id || !date) {
      ctx.body = {
        code: 400,
  ...
  async data() {
    ...
    try {
      ...
      let total_data = _data.reduce((arr, cur) => {
        const index = arr.findIndex(item => item.label_id === cur.label_id);
        if (index === -1) {
          arr.push({
