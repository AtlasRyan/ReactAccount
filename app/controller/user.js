...
class UserController extends Controller {
  async register() {
    const { ctx } = this;
    const { user_name, password } = ctx.request.body; // 获取注册需要的参数
    // 判空操作
    ...
    // 验证数据库内是否已经有该账户名
    const userInfo = await ctx.service.user.getUserByName(user_name);// 获取用户信息
  ...
  async login() {
    ...
    const userInfo = await ctx.service.user.getUserByName(user_name);
    // 没找到说明没有该用户
    if (!userInfo || !userInfo.id) {
      ctx.body = {
        code: 500,
  ...
  async modify_pass() {
    const { ctx, app } = this;
    // 通过 post 请求，在请求体中获取签名字段 signature
    const { old_password, new_password, new_password2 } = ctx.request.body;
